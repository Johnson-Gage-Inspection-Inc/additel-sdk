...
class TestSetupConfiguration(unittest.TestCase):
    def setUp(self):
        ...
        self.temp_readme = tempfile.NamedTemporaryFile(
    ...
    def test_setup_config(self):
        ...
        self.assertEqual(
            self.recorded_kwargs.get("author_email"), "rhythmatician5@gmail.com"
        )
        self.assertEqual(
            self.recorded_kwargs.get("description"),
            "A Python SDK to communicate with Additel devices",
        )
        self.assertIn("README", self.recorded_kwargs.get("long_description", ""))
        self.assertEqual(
            self.recorded_kwargs.get("long_description_content_type"), "text/markdown"
        )
