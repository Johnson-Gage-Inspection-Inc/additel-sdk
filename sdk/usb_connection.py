...
class USBConnection:
    ...
    def __init__(self, parent, **kwargs):
        ...
        if not self.vendor_id or not self.product_id:
            raise ValueError("Both 'vendor_id' and 'product_id' must be specified for USB connection.")

        self.backend = libusb1.get_backend(find_library=lambda x: self.backend_path) if self.backend_path else None

    def connect(self):
        """Establish connection to the USB device."""
        try:
            self.device = usb.core.find(idVendor=self.vendor_id, idProduct=self.product_id, backend=self.backend)
            if self.device is None:
                raise ConnectionError("USB device not found. Ensure the device is connected and IDs are correct.")
            ...
            if self.endpoint_out is None or self.endpoint_in is None:
                raise ConnectionError("Endpoints for USB communication could not be established.")
    ...
    def send_command(self, command):
        ...
        if not self.device or not self.endpoint_out:
            raise ConnectionError("USB device is not connected or output endpoint is unavailable.")
    ...
    def read_response(self, timeout=5000):
        ...
        if not self.device or not self.endpoint_in:
            raise ConnectionError("USB device is not connected or input endpoint is unavailable.")

        try:
            response = self.device.read(self.endpoint_in.bEndpointAddress, self.endpoint_in.wMaxPacketSize, timeout=timeout)
